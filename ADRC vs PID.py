...
model = collimator.load_model("Object - ADRC")
...
# b0 calculated from nominal mass (10 kg)
b0 = 1/10
...
for idx in range(len(m_V)):
    plt.plot(res_V[idx].index, res_V[idx]["Object0.Position"], label=f"Mass = {m_V[idx]} kg")
...
for idx in range(len(m_V)):
    plt.plot(res_V[idx].index, res_V[idx]["Saturate_0.out_0"], label=f"Mass = {m_V[idx]} kg")
